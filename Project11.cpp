...
int main()
{
	...
	for(int i=0;i<20;i++)
	{
		for(int j=0;j<20;j++)
		{
			...
			if(j>=3)
			{
				long long temp=arr[i][j]*arr[i][j-2]*arr[i][j-1]*arr[i][j-3];
				max=(temp>max)?temp:max;
			}
			if(j<=16)
			{
				long long temp=arr[i][j]*arr[i][j+1]*arr[i][j+2]*arr[i][j+3];
				max=(temp>max)?temp:max;
			}
			if(i<=16&&j<=16)
			{
				long long temp=arr[i][j]*arr[i+1][j+1]*arr[i+2][j+2]*arr[i+3][j+3];
				max=(temp>max)?temp:max;
			}
			if(i<=16&&j>=3)
			{
				long long temp=arr[i][j]*arr[i+3][j-3]*arr[i+2][j-2]*arr[i+1][j-1];
				max=(temp>max)?temp:max;
			}
			if(i>=3&&j>=3)
			{
